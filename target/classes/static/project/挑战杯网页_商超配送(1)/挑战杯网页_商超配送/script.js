...
function initMainNav() {
    ...
    navItems.forEach(item => {
        item.addEventListener('click', function(e) {
            ...
            if(targetPage==='home'){
                window.location.href='../../挑杯_社区管理 (2)/挑杯_社区管理/index.html'
            }

            // 特殊处理社区物业跳转
            if (targetPage === 'property') {
                // 跳转到社区物业页面
...
function showMainNav() {
    // 修改：主导航栏始终显示，不需要.active类控制
    document.getElementById('main-nav').style.display = 'flex';
    document.getElementById('nav-user-info').textContent = currentUser || '未登录';
}
...
function showLoginPage() {
    hideAllPages();
    hideAllFooters();
    // 登录页面也显示主导航，但用户信息显示"未登录"
    document.getElementById('main-nav').style.display = 'flex';
    document.getElementById('nav-user-info').textContent = '未登录';
...
function bindEvents() {
    ...
    if (document.getElementById('manage-products-btn')) {
    ...
    // 管理员功能按钮
    if (document.getElementById('manage-all-products-btn')) {
    ...
    // 添加商品表单提交
    if (document.getElementById('submit-product-btn')) {
...
function searchProducts(keyword) {
    ...
    if (currentRole === 'customer') {
        renderSearchResults(filteredProducts, 'products-container');
    }
}

// 渲染搜索结果
function renderSearchResults(filteredProducts, containerId) {
